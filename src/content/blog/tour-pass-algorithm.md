...
这不是为了证明数学上最优，而是为了在性能、可解释性和路线质量之间取一个平衡。当前参数下，时间槽数量固定，Beam 宽度和分支数也有限，所以本地样例数据可以稳定快速返回。
...
| 接口 | avg | p95 |
| --- | ---: | ---: |
| `GET /health` | 0.6 ms | 0.9 ms |
| `GET /route/shortest` | 0.5 ms | 0.7 ms |
| `GET /poi/search` | 0.7 ms | 1.1 ms |
| `POST /trip/plan` | 418.3 ms | 436.7 ms |

基准运行时会强制 `LLM_DISABLED=1`，避免远程模型网络波动污染结果。

这个数据不是为了证明“性能极限很强”，而是为了建立回归基线。以后改 Beam Search、评分函数或候选数量时，至少能知道有没有把响应时间拖坏。
