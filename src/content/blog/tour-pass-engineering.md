...
CI 之外，项目现在还增加了 Docker 构建和容器冒烟路径。`Dockerfile` 使用 Linux 容器内的 CMake Release 构建服务，容器默认关闭远程 LLM，适合公开视频和面试复现。GitHub Actions 在 PR 中可以构建镜像并运行 `scripts/container_smoke.js`；`main` 分支则预留 GHCR 推送路径。部署文档也只按“Docker Web Service 草案”表达，除非真的拿到公网 URL，否则不声称有线上 Demo。
