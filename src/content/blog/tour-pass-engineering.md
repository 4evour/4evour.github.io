...
description: "记录 Tour Pass 怎么从算法代码变成稳定演示项目：Makefile/CMake、数据质量门禁、CI、API 冒烟、Web 演示台、LLM 兜底和性能基准。"
pubDate: 2026-05-18
tags: ["C++", "工程化", "CI", "项目复盘"]
project: "Tour Pass"
featured: true
...
如果这个项目要放进简历和面试展示，它还需要几件事：能稳定构建，能自动测试，数据坏了能发现，API 能冒烟验证，页面能展示算法过程，远程 LLM 不可用时也能讲得下去。
...
报告会记录 `/health`、`/route/shortest`、`/poi/search` 和 `/trip/plan` 的 avg、p50、p95、min、max。

当前样例规模下，规划接口大约是 400 多毫秒，其他查询接口是毫秒级。这个数字的意义不是“压测很厉害”，而是建立一个回归基线。

如果以后我改 Beam Search 参数、增加候选策略、扩大 POI 数据集，性能报告能告诉我：这次改动有没有把演示体验拖慢。
...
它没有数据库，没有容器编排，也没有接真实地图服务。相反，它把精力放在这个阶段最重要的几件事上：本地稳定运行、数据质量、API 可验证、演示可视化、远程依赖可降级、性能有基线。
