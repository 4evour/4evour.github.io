...
2026-05-23 更新：这条工程化链路已经从“本地样例能稳定演示”继续补到“真实数据、容器、部署和录屏证据”。新增的能力不是为了把项目包装成生产平台，而是为了让作品集表达更可信：数据从哪里来、哪些边还是估算、容器能不能启动、性能数字是什么口径，都有可复现命令支撑。

2026-06-18 更新：Tour Pass 后续又补了 Python Agent、React 行程编辑器、用户认证、Render 线上演示和 `/agent/*` 反向代理修复。本文下面仍保留 C++ 工程化阶段的复盘，新平台化口径见《Tour Pass 6 月更新：从 C++ 算法服务到 AI 行程平台》。
...
真实数据流水线沿用了同一套门禁。`scripts/run_real_data_pipeline.js` 可以串起 POI 采集、通勤边生成、数据校验和真实规模实验；严格口径下可以要求 `--fallback fail` 和 `--min-amap-ratio 0.8`，让高德来源边比例不够时直接失败。对外展示时，我更愿意把这件事讲清楚：脚本不是为了制造一个漂亮数字，而是为了防止“真实 POI + 大量估算边”被误说成真实路网。
...
CI 之外，当时项目还增加了 Docker 构建和容器冒烟路径。`Dockerfile` 使用 Linux 容器内的 CMake Release 构建服务，容器默认关闭远程 LLM，适合公开视频和面试复现。GitHub Actions 在 PR 中可以构建镜像并运行 `scripts/container_smoke.js`；`main` 分支则预留 GHCR 推送路径。部署文档当时只按“Docker Web Service 草案”表达，除非真的拿到公网 URL，否则不声称有线上 Demo。6 月后仓库 README 已经补上 Render 线上演示入口。
...
真实规模实验也走同样的表达边界。当前本机真实数据报告记录过一次 `500 POI / 1937 edges` 的长沙数据集，原始高德来源边比例是 `88.1%`；随后用 `scripts/retry_geo_edges.js` 重试 `geo_estimated` 边，把高德来源比例提升到 `98.5%`，但仍保留 `29` 条估算边并明确披露。`docs/scale_experiment_report.md` 中 100/200/500 POI 的 p95 大约分别是 `6.5 ms`、`6.3 ms`、`128.9 ms`，这些数字只说明本地回归趋势，不代表生产 SLA。

为了让别人不用编译也能快速看懂项目，我还补了一份 `docs/demo_recording_checklist.md`。录屏不追求把所有命令塞满，而是按“项目边界 -> 真实数据证据 -> Web 演示台 -> 算法/规模报告”的顺序讲清楚。博客里的演示视频也会采用这个思路，只是把重点放在浏览器里的候选对比、路线明细、算法解释和工具箱。
