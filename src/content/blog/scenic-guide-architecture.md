...
2026-06-18 更新：这篇保留系统第一版架构复盘。6 月版本继续补了官方资料知识库、五种 RAG 检索模式、真实流式回答、Cookie/CSRF 鉴权、电子围栏、老年模式、二维码管理和游客反馈分析闭环；最新内容单独写在《景区导览系统 6 月更新：从演示链路到游客闭环》。
...
![灵山智能导览数据大屏](/images/blog/scenic-guide/dashboard.png)

![灵山智能导览游客地图](/images/blog/scenic-guide/map.png)
...
前端用了 Vue 3 + Vite + TypeScript。管理后台、游客实时地图、运营数据大屏和数字人备用视图都在 `web-vue/` 里。
...
前端视觉上我做了一套 `--sg-*` 设计 token，想让它更像“文旅国风科技”，而不是默认后台模板。比如数据看板是深色底，青色科技线和金色文化强调一起用；管理页、地图页和数字人备用视图也尽量复用同一套变量。
...
现在这版当然还可以继续变强，比如引入 Redis、补更多端到端测试、完善监控和把部署做成容器化流水线。但作为一个阶段性作品，它已经能比较完整地展示我对全栈系统、AI 接入和工程边界的理解。
